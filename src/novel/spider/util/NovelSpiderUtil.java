...
public final class NovelSpiderUtil {
	...
	//上传到Linux服务器   /usr/local/tomcat/app/conf/Spider-Rule.xml
	//在本地服务器  /conf/Spider-Rule.xml
	private static String confPath="E:\\apache-tomcat-7.0.92\\webapps\\ssm18\\WEB-INF\\classes\\conf\\Spider-Rule.xml";
	...
	private static void init() {
		...
		System.out.println("NovelSpiderUtil void init()  is going on");
		try {
			Document doc = reader.read(new File(confPath));
			//System.out.println("NovelSpiderUtil ' s doc =  " + doc);
...
//			Document doc = reader.read(new File("/home/williams/novel/conf/Spider-Rule.xml"));
			//System.out.println("未执行或执行时出错 Element root = doc.getRootElement();");
			Element root = doc.getRootElement();
		//	System.out.println("root=="+root);
			List<Element> sites = root.elements("site");
		//	System.out.println("sites"+sites.get(1));
			for (Element site : sites) {
				...
				for (Element sub : subs) {
					...
					System.out.println("NovelSpiderUtil ' s subMap="+subMap);
				}
				System.out.println("NovelSpiderUtil yunxingchenggongle");
	...
	public static Map<String, String> getContext(NovelSiteEnum novelSiteEnum) {
		System.out.println("NovelSpiderUtil 's  Map<String, String> getContext 运行了");
		
		if(CONTEXT_MAP.isEmpty()){
			init();
		}
		//return CONTEXT_MAP.get(novelSiteEnum);
		return CONTEXT_MAP.get(novelSiteEnum.Bxwx);
	}
