...
public class BxwxNovelInfoSpider extends AbstractNovelInfoSpider{
	...
	public NovelInfo getNovelInfo(String url) {
		try {
			System.out.println("BxwxNovelInfoSpider 's NovelInfo's getNovelInfo 运行了");
			System.out.println("下一步执行或执行时出错BxwxNovelInfoSpider 's String result = super.crawl(url)");
			...
			doc.setBaseUri(url);
			String novelUrl=url;
			System.out.println("BxwxNovelInfoSpider 's novelUrl = "+novelUrl+"docsub = "+doc.toString().substring(0,20));
